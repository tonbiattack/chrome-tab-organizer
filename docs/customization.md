...
- `グループ名`: タブグループ名
- `グループ色`: 横並びのカラーパレットから選択する Chrome タブグループ色
...
popup では上記の色を横並びパレットで表示し、色見本を見ながら選択できます。
...
`標準ルールのグループだけ解除` ボタンは、標準ルール名と一致するタイトルのタブグループだけを解除します。カスタムルール名と重複している標準名は対象から除外します。

これらの解除は、現在開いているタブグループだけでなく、ブックマークバーに保存されていて未オープンの同名グループ（フォルダ）も対象です。
