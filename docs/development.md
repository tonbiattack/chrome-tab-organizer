...
- 保存済みグループ解除には `chrome.bookmarks` も使います
...
- `ungroupAllTabs` はブックマークバーも見て、ルール名と一致するタイトルの保存済みフォルダも削除する
- popup で `chrome.storage` を使うため、`manifest.json` に `storage` 権限が必要
- 保存済みグループ解除のため、`manifest.json` に `bookmarks` 権限が必要
...
- ブックマークバー上の通常フォルダでもルール名と同名なら削除対象になる
