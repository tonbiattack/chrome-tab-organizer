...
- 保存や復元は `Session Buddy` など別ツールに任せ、この拡張は復元後のタブ整理を担う想定でも使えます
...
8. `グループをすべて解除` を押し、開いているグループがすべて解除されることを確認する
...
- グループ化処理では、開いている既存グループをいったんすべて解除してから再作成する
- `ungroupAllTabs` は開いているグループをすべて解除する
