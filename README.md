...
標準ルールによるグルーピングに加えて、popup から複合カスタムルールを追加し、Jira の課題キーや Google Doc ID、任意の URL パターンを組み合わせて整理できます。
...
- popup から `Jira課題キー` `Google Doc ID` `URLパターン` を自由に組み合わせたルールを追加できます
...
- 標準ルールの有効 / 無効
- Jira 課題キー / Google Doc ID / URL パターンを組み合わせたカスタムルール追加
- 既存カスタムルールの有効 / 無効と削除
- カスタムルールのインポート / エクスポート
- `グループをすべて解除` による拡張管理グループの解除
- `標準ルールのグループだけ解除` による標準ルールグループのみの一括解除
...
| `bookmarks` | ブックマークバーに保存されたタブグループの削除 |
...
- `標準ルールのグループだけ解除` は、カスタムルール名と重複する標準名を除外したうえで標準ルール名と一致するグループだけを対象にします
- 上記 2 つの解除は、現在開いているタブグループに加えて、ブックマークバーに保存されていて未オープンのグループ（同名フォルダ）も対象にします
