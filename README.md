...
- 拡張が管理するグループだけを解除する
- ブックマークバー上の保存済み managed group も解除する
...
| `bookmarks` | ブックマークバー上の保存済みグループ解除 |
...
- `グループをすべて解除` は、標準ルール名またはカスタムルール名と一致する開いているグループと、ブックマークバー上の保存済みフォルダだけを対象にします
- 手動で作ったグループでも、ルール名と同じタイトルを付けると管理対象として扱われます
- 保存済みグループの削除判定もタイトル一致ベースです。ブックマークバー上の通常フォルダでもルール名と同名なら対象になります
